...
coalesce:function(){var a;for(a=0;a<arguments.length;a++)if(!this.isNothing(arguments[a]))return arguments[a];return null},registerNamespace:function(){var a=arguments,b=null,c,d;for(c=0;c<a.length;++c){var f=a[c].split(".");d=f[0];eval("if (typeof "+d+' == "undefined"){'+d+" = {};} obj = "+d+";");for(d=1;d<f.length;++d)b[f[d]]=b[f[d]]||{},b=b[f[d]]}},extend:function(a,b,c){this.isNothing(c)&&(c=!0);if(a&&b&&this.isObject(b))for(var d in b)c?a[d]=b[d]:typeof a[d]=="undefined"&&(a[d]=b[d])},swapArrayElements:function(a,
b,c){var d=a[b];a[b]=a[c];a[c]=d},isObject:function(a){return typeof a=="object"},isNothing:function(a){if(typeof a==="undefined"||a===null)return!0;return!1},isFunction:function(a){return typeof a=="function"},isArray:function(a){return a&&Code.PhotoSwipe.Util.isFunction(a.pop)},isNumber:function(a){return typeof a=="number"},isString:function(a){return typeof a=="string"},trim:function(a){return a.replace(RegExp(/\s+?/),"")}};if(Code.PhotoSwipe.Util.browser.webkit){var e=document.createElement("div");
Code.PhotoSwipe.Util.browser.is3dSupported=!Code.PhotoSwipe.Util.isNothing(e.style.WebkitPerspective)}Code.PhotoSwipe.Util.browser.touchSupported=Code.PhotoSwipe.Util.browser.isEventSupported("touchstart");Code.PhotoSwipe.Util.browser.gestureSupported=Code.PhotoSwipe.Util.browser.isEventSupported("gesturestart")})(window);
(function(e,a){a.extend(a,{DOM:{resetTranslate:function(b){a.browser.webkit?a.browser.is3dSupported?a.DOM.setStyle(b,{webkitTransform:"translate3d(0px, 0px, 0px)"}):a.DOM.setStyle(b,{webkitTransform:"translate(0px, 0px)"}):a.DOM.setStyle(b,{webkitTransform:"translate(0px, 0px)",MozTransform:"translate(0px, 0px)",transform:"translate(0px, 0px)"})},createElement:function(a,c,d){var a=document.createElement(a),f;for(f in c)c.hasOwnProperty(f)&&a.setAttribute(f,c[f]);a.innerHTML=d||"";return a},appendChild:function(a,
...
a){if(this._listeners[e]instanceof Array)for(var b=this._listeners[e],c=0,d=b.length;c<d;c++)if(b[c]===a){b.splice(c,1);break}}})})();
...
"onhashchange"in e;this.viewportFadeInEventHandler=this.onViewportFadeIn.bind(this);this.windowOrientationChangeEventHandler=this.onWindowOrientationChange.bind(this);this.windowScrollEventHandler=this.onWindowScroll.bind(this);this.windowStateChangeHandler=this.onWindowStateChange.bind(this);this.keyDownEventHandler=this.onKeyDown.bind(this);this.viewportTouchEventHandler=this.onViewportTouch.bind(this);this.viewportFadeOutEventHandler=this.onViewportFadeOut.bind(this);this.sliderDisplayCurrentFullSizeImageEventHandler=
this.onSliderDisplayCurrentFullSizeImage.bind(this);this.toolbarClickEventHandler=this.onToolbarClick.bind(this)},setOptions:function(b){a.extend(this.settings,b)},setImages:function(b){if(!a.isArray)throw"thumbEls is not an array";this.currentIndex=0;this.fullSizeImages=[];for(var c=0;c<b.length;c++){var e=b[c];this.fullSizeImages.push(new d(c,this.settings.imageScaleMethod,this.settings.getImageSource(e),this.settings.getImageCaption(e),this.settings.getImageMetaData(e)))}},show:function(c){if(!this.isBusy){if(!a.isNumber(c))throw"startingIndex must be a number";
if(a.isNothing(this.fullSizeImages))throw"need to set images before showing the gallery";this.isBusy=!0;if(this.isBackEventSupported)this.originalHashValue=e.location.hash;this.lastShowPrevTrigger=Code.PhotoSwipe.ShowPrevTriggers.show;a.DOM.addClass(document.body,Code.PhotoSwipe.CssClasses.activeBody);c=e.parseInt(c);if(c<0||c>=this.fullSizeImages.length)c=0;this.currentIndex=c;a.isNothing(this.documentOverlay)?this.build():this.resetPosition();this.viewport.addEventListener(b.EventTypes.onFadeIn,
...
new g({fadeInSpeed:this.settings.fadeInSpeed,fadeOutSpeed:this.settings.fadeOutSpeed,slideSpeed:this.settings.slideSpeed},this.viewport.el);this.captionAndToolbar=new j({opacity:this.settings.captionAndToolbarOpacity,fadeInSpeed:this.settings.fadeInSpeed,fadeOutSpeed:this.settings.fadeOutSpeed,autoHideDelay:this.settings.captionAndToolbarAutoHideDelay,flipPosition:this.settings.captionAndToolbarFlipPosition,showEmptyCaptions:this.settings.captionAndToolbarShowEmptyCaptions,hideClose:this.settings.preventHide,
zIndex:this.settings.zIndex+3});this.resetPosition()},addEventListeners:function(){this.orientationEventName=a.browser.isAndroid?"resize":"onorientationchange"in e?"orientationchange":"resize";a.DOM.addEventListener(e,this.orientationEventName,this.windowOrientationChangeEventHandler);a.DOM.addEventListener(e,"scroll",this.windowScrollEventHandler);if(this.isBackEventSupported)this.currentHistoryHashValue="lookbook"+(new Date).getTime().toString(),e.location.hash=this.currentHistoryHashValue,a.DOM.addEventListener(e,
"hashchange",this.windowStateChangeHandler);a.DOM.addEventListener(document,"keydown",this.keyDownEventHandler);this.viewport.addEventListener(f.EventTypes.onTouch,this.viewportTouchEventHandler);this.slider.addEventListener(g.EventTypes.onDisplayCurrentFullSizeImage,this.sliderDisplayCurrentFullSizeImageEventHandler);this.captionAndToolbar.addEventListener(i.EventTypes.onClick,this.toolbarClickEventHandler)},removeEventListeners:function(){a.DOM.removeEventListener(e,this.orientationEventName,this.windowOrientationChangeEventHandler);
a.DOM.removeEventListener(e,"scroll",this.windowScrollEventHandler);this.isBackEventSupported&&a.DOM.removeEventListener(e,"hashchange",this.windowStateChangeHandler);a.DOM.removeEventListener(document,"keydown",this.keyDownEventHandler);this.viewport.removeEventListener(f.EventTypes.onTouch,this.viewportTouchEventHandler);this.slider.removeEventListener(g.EventTypes.onDisplayCurrentFullSizeImage,this.sliderDisplayCurrentFullSizeImageEventHandler);this.captionAndToolbar.removeEventListener(i.EventTypes.onClick,
...
this.showNext();break;case i.Actions.play:this.startSlideshow();break;default:this.hide()}},startSlideshow:function(){if(!this.isBusy)a.isNothing(this.slideshowTimeout)||e.clearTimeout(this.slideshowTimeout),this.removeZoomPanRotate(),this.isSlideshowActive=!0,this.fadeOutCaptionAndToolbar(),this.fireSlideshowTimeout(),this.dispatchEvent(Code.PhotoSwipe.EventTypes.onSlideshowStart)},stopSlideshow:function(){a.isNothing(this.slideshowTimeout)||e.clearTimeout(this.slideshowTimeout);this.isSlideshowActive=
!1;this.dispatchEvent(Code.PhotoSwipe.EventTypes.onSlideshowStop)},fireSlideshowTimeout:function(){var a=!1;this.settings.loop?this.fullSizeImages.length>1&&(a=!0):this.currentIndex<this.fullSizeImages.length-1&&(a=!0);if(a)this.lastShowPrevTrigger=Code.PhotoSwipe.ShowPrevTriggers.slideshow,this.slideshowTimeout=e.setTimeout(this.showNext.bind(this),this.settings.slideshowDelay)},removeZoomPanRotate:function(){if(!a.isNothing(this.zoomPanRotate))this.zoomPanRotate.removeFromDOM(),this.zoomPanRotate=
